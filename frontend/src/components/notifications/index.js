...
class Notifications extends Component {
  ...
  createElement (props) {
    ...
    return html`
      <div class="notifications-component fixed flex flex-auto z-max">
  ...
  renderNotifications () {
    const notifications = this.local.items.map(item => {
      ...
      return html`
        <li class="${fg} ${borders} flex flex-auto items-center pv1 pr4 mb2 message ${type}">
          <span class="flex items-center justify-center h3 w3">
            ${icon(iconName, { class: type === 'success' ? 'fill-green' : iconFillInvert, size: 'sm' })}
          </span>
          <p class="lh-copy pl2 lh-copy f5 mw5">
            ${message}
          </p>
        </li>
      `
